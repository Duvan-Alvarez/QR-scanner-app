...
if (!existingAdmin) {
  db.prepare('INSERT OR IGNORE INTO users (username, password, role, active) VALUES (?, ?, ?, ?)').run(defaultAdminUser, defaultAdminPassword, 'admin', 1);
}

const existingScanner = db.prepare('SELECT * FROM users WHERE username = ?').get(defaultScannerUser);
if (!existingScanner) {
  db.prepare('INSERT OR IGNORE INTO users (username, password, role, active) VALUES (?, ?, ?, ?)').run(defaultScannerUser, defaultScannerPassword, 'scanner', 1);
}
...
const validCodeColumns = db.prepare("PRAGMA table_info(valid_codes)").all();
if (!validCodeColumns.some((column) => column.name === 'active')) {
  db.prepare('ALTER TABLE valid_codes ADD COLUMN active INTEGER NOT NULL DEFAULT 1').run();
}
